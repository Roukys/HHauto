...
function clearHaremToolVariables()
{
...
function HaremDisplayGirlPopup(haremItem,haremText,remainingTime)
{
    ...
    document.getElementById("popup_message_harem_close").addEventListener("click", function() {
        clearHaremToolVariables();
        location.reload();
    });
}
function HaremClearGirlPopup()
{
    $("#popup_message_harem").each(function(){this.remove();});
}

function HaremUpdateGirlPopup(haremItem,haremText,remainingTime)
{
    HaremClearGirlPopup();
    HaremDisplayGirlPopup(haremItem,haremText,remainingTime);
}
...
function moduleHaremGirl()
{
    ...
    try {
        ...
        if(haremGirlMode === 'girl' && haremGirlLimit && (Number(girl.level) + 50) <= Number(haremGirlLimit))
        {
            ...
            HaremDisplayGirlPopup(haremItem, girl.name + ' '+girl.Xp.cur+"xp, level "+girl.level+"/"+haremGirlLimit, (1)*5 );
...
var autoLoop = function ()
{
    ...
    if (burst && !mouseBusy /*|| checkTimer('nextMissionTime')*/)
    {
        ...
        */
        ...
        if (busy === false && getHHScriptVars("isEnabledGreatPachinko",false) && getStoredValue("HHAuto_Setting_autoFreePachinko") === "true" && getStoredValue("HHAuto_Temp_autoLoop") === "true" && checkTimer("nextPachinkoTime") && canCollectCompetitionActive()) {
            ...
            busy = getFreePachinko('great','nextPachinkoTime','great_pachinko_timer');
        }
        ...
        if (busy === false && getHHScriptVars("isEnabledMythicPachinko",false) && getStoredValue("HHAuto_Setting_autoFreePachinko") === "true" && getStoredValue("HHAuto_Temp_autoLoop") === "true" && checkTimer("nextPachinko2Time") && canCollectCompetitionActive()) {
            ...
            busy = getFreePachinko('mythic','nextPachinko2Time', 'mythic-timer');
        }
        ...
        if (busy === false && getHHScriptVars("isEnabledEquipmentPachinko",false) && getStoredValue("HHAuto_Setting_autoFreePachinko") === "true" && getStoredValue("HHAuto_Temp_autoLoop") === "true" && checkTimer("nextPachinkoEquipTime") && canCollectCompetitionActive()) {
            ...
            busy = getFreePachinko('equipment','nextPachinkoEquipTime', 'equipment-timer'); // No timer. yet ?
        }
    ...
    switch (getPage())
    {
        ...
        case getHHScriptVars("pagesIDHome"):
            ...
            clearHaremToolVariables = callItOnce(clearHaremToolVariables); // Avoid wired loop, if user reach home page, ensure temp var from harem are cleared
            clearHaremToolVariables();
            break;
        case getHHScriptVars("pagesIDHarem"):
            moduleHarem();
...
var hhTimerLocale = unsafeWindow.Phoenix ? unsafeWindow.Phoenix.language : null;
var timerDefinitions;
if (unsafeWindow.Phoenix && (hhTimerLocale !== undefined || hhTimerLocale !== null || hhTimerLocale.length > 0)) {
    timerDefinitions = {
    [hhTimerLocale]: {
