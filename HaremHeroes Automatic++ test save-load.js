// ==UserScript==
// @name         HaremHeroes Automatic++ test save-load
...
GM_addStyle('.myButton {margin:2px;box-shadow: 0px 0px 0px 2px #9fb4f2; background:linear-gradient(to bottom, #7892c2 5%, #476e9e 100%); background-color:#7892c2; border-radius:10px; border:1px solid #4e6096; display:inline-block; cursor:pointer; color:#ffffff; font-family:Arial; font-size:8px; padding:3px 10px; text-decoration:none; text-shadow:0px 1px 0px #283966;}.myButton:hover { background:linear-gradient(to bottom, #476e9e 5%, #7892c2 100%); background-color:#476e9e; } .myButton:active { position:relative; top:1px;}');
GM_addStyle('.HHEventPriority {width:52px;bottom:-20px;background:black;border: 2px solid #fff;cursor:default;position:absolute;text-shadow:0px 0px 6px red;color: white;text-align: center;}');//changed
GM_addStyle('.HHShopInfoAd{position:absolute;top:23px;left:55%;margin:0;margin-left:-75px;padding:05px;width:150px;text-align:center;border-radius:6px;font-size:16px;line-height:38px;overflow:hidden;text-overflow:ellipsis;height:38px;white-space:nowrap;z-index:20;background:#fff;color:#057;}');
GM_addStyle('#LoadDialog:not([open]){display: none;} #LoadDialog{font-size:14px;}');
//END CSS Region
...
function getSuitableMission(missionsList)
{
    ...
    {
        if(Number(msn.duration) < Number(missionsList[m].duration)) //changed
        {
            msn = missionsList[m];
        }
    }
...
function getGirlsJson() {
    return unsafeWindow.girlsDataList;
}
...
var updateData = function () {
    ...
    if ($('#LoadDialog[open]').length > 0) {return} //changed
    ...
    //document.getElementById("autoExp").checked = Storage().autoExp === "58000000";
    ...
    if (Storage().showInfo=="true")
    {
        ...
        */
        ...
        if (Tegzd.length>0)
        {
            //Shortcut to set timer to 1sec
            //document.getElementsByClassName('hh_logo')[0].addEventListener("click", function(){setTimer('nextSeasonTime',Number(1));},true); //changed 2
...
function moduleSimSeasonBattle() {
    ...
    try
    {
        ...
        for (index=0;index<3;index++)
        {
            ...
            else if (chosenFlag == currentFlag && currentFlag == -1 && currentScore > chosenRating)
            {
                //console.log('second');
                chosenRating = currentScore;
                chosenFlag = currentFlag;
...
var autoLoop = function () {
    ...
    if (burst /*|| checkTimer('nextMissionTime')*/)
    {
        ...
        */
        ...
        if (getPage()==="harem") { //changed
            // Storage().girlJson = unsafeWindow.girlsDataList;
            // console.log("Page:::"+getPage()+' girls:'+Object.keys(girlJson).length);
        }
...
{
    if ($('.HHEventPriority').length  > 0) {return}//changed
    $('#homepage div.event-widget.special-fullscreen-view div.widget div.container div.scroll-area')[0].style.overflow="visible";//changed
    ...
    if ( eventGirlz.length >0 )
    {
        ...
        $("div.event-widget div.widget[style='display: block;'] div.container div.scroll-area div.rewards-block-tape div.girl_reward").each(function()
            ...
            if ( currentGirl in widgetGirlz )
            {
                ...
                //if ($(this).find('.HHEventPriority').length == 0) //changed
                {
...
function saveAsJSON(data, name='HH_'+Date.now()+'.json') { //changed
...
function onChange(event) { //changed lastedit
...
function onReaderLoad(event){ //Changed
...
var start = function () {
    ...
    UIcontainer.html( '<div style="font-size:x-small;position: absolute;right: 22%;width: inherit;text-align: center;display:flex;flex-direction:column;z-index:1000" id="sMenu">'
                     ...
                     +   '<span><div><label class=\"myButton\" id=\"git\">GitHub</label></div></span>'
    ...
    div.innerHTML = '<div id="pInfo" style="font-size:14px;padding-left:3px;z-index:-1;white-space: pre;position: absolute;right: 5%; left:75%; top:8%;border: 1px solid #ffa23e;background-color: rgba(0,0,0,.5);border-radius: 5px;"></div>'.trim();
    ...
    document.getElementById("loadConfig").addEventListener("click", function(){//changed
    ...
    document.getElementById('myfile').addEventListener('change', onChange);//changed
    document.getElementById("saveConfig").addEventListener("click", function(){//changed
        saveAsJSON(Storage());
    });
