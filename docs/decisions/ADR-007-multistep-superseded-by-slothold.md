# ADR-007: Multi-Step-Zerlegung durch Slot-Hold abgeloest
...
## Kontext
Geplant waren vier Handler-Zerlegungen: PlaceOfPower,
...
Diese Tasks wurden VOR ADR-005 (Slot-Hold) spezifiziert. Ihr Zweck war
...
Stand 2026-06-14, nach ADR-005 + dem No-Progress-Watchdog-Fix (v7.36.11): der
...
- **ChampionTicket:** der Doppelkauf-Race ist ueber `autoLoop=false` VOR dem
  ...
  (zwei Writes decken zwei Race-Fenster).

## Entscheidung
Die vier Zerlegungen werden NICHT umgesetzt. Sie sind
durch ADR-005 (Slot-Hold) + den Watchdog-Fix abgeloest. Die Ziel-Outcomes
...
### Alle vier voll umsetzen (explizite Multi-Step-Bloecke)
...
### Nur Quest umsetzen, Rest schliessen
...
- Verworfen fuer 7.37.0; als optionales Cleanup ins Bugfix-Release nach 7.37.0
  verschiebbar (zusammen mit dem lastActionPerformed-Umbau).

## Konsequenzen
- Die Block-Architektur ist funktional abgeschlossen; die vier Zerlegungen
  entfallen ersatzlos.
...
## Referenzen
- ADR-005 (Slot-Hold), ADR-004 (Block-Modell).
