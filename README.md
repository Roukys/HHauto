...
### v7.35.32 - League team builder rebuild: Best Possible without max(), element synergy, Shield-first leader, blessing diagnostics

The team builder for "Current Best" / "Best Possible" was reviewed end-to-end based on logs and dumps from issue #1603. The mode semantics were tightened, the EffectivePower formula now reflects four contributors instead of one, and every team selection now logs enough detail to verify the choice without guessing.

- **Best Possible without `max()` guard.** Mode 2 now consistently projects every eligible girl to the awakening cap (level 750, all grades). The old `max(projected, current)` clamp made Mode 2 collapse into Mode 1 whenever the player's top picks were already maxed-out -- the two buttons looked identical even though that was not the intent. With game data on a normal range (`level <= 750`, `graded <= nb_grades`), `projected` is mathematically always >= `current`, so the clamp was redundant. Mode 2 now ranks low-level mythics by their cap potential, which is what "Best Possible" means.
- **Mode pool unchanged.** Both modes still draw from the same Mythic + Legendary 5* pool of the player's class. The change is in the score function, not in eligibility.
- **EffectivePower = BaseStats × (1 + ElementSynergy) × (1 + Tier3) × (1 + LeaderBonus).** The cluster comparison used to compare on `mainSum * (1 + Tier3)` only. Two more factors are now in the formula: the team's element-stack bonus (mono-fire = +70%, mixed teams less) and a leader-role bonus tied to the Tier-5 priority (Shield = +8%, Stun = +6%, Execute = +4%, Reflect = +2%). With ElementSynergy in the picture, a 6/7 mono-element cluster team beats a 4+2 mix at the same Tier-3, which mirrors the actual game behaviour of theme synergies.
- **Shield-first leader, even after the slot fill.** Frank's request from issue #1573 is now hard: when slots 2-7 are filled and the leader candidate has no Shield mythic left in the rest pool, the strongest Shield mythic from the slots is swapped out and used as leader, with the strongest remaining legendary taking the freed slot. Stun -> Execute -> Reflect remain as fallback only.
- **Blessing detection from the game-authoritative field.** `BlessingService.getEffectiveMultiplier(girl)` reads `blessing_bonuses.pvp_v3.carac1` (with `pvp_v4` fallback) directly from the per-girl game data. `pvp_v3 = [25]` -> 1.25x, `pvp_v3 = [40]` -> 1.40x, `pvp_v3 = [40, 25]` -> 1.75x. The girl's `caracs` already carries the blessed value, so the score is correct; the new helpers expose the raw percent list for the audit and info box. (Old keyword scanner on `pvp_v3`/`pvp_v4` keys was a no-op.)
- **Per-slot diagnostics in the log.** Every `setTopTeamV2` call now writes a second log line listing each picked girl with `level`, `awakening_level`, `graded/nb_grades`, score, trait value, cluster membership, and active blessing percents. Combined with the new `EffPower=`/`Synergy=`/`LeaderBonus=`/`pool: ... (X mythics, Y at cap, Z blessed)` summary line, a single log file shows exactly why the modes converged or diverged for the player's roster, without needing a dump.
- **Audit annotates blessed mythics.** The "Excluded mythics" list in the info box adds e.g. `+25%` or `+40/25%` next to the stat number, so a strongly blessed mythic that was filtered out of the team is immediately visible. The pool stat line in the info box additionally shows how many own-class mythics are currently at the level cap and how many carry an active blessing.
- **Modes can still converge mathematically.** When every Top-7 mythic is at level 750 with full grades and full awakening, projected stat per girl equals current stat per girl, so both modes pick the same team. The new logging makes this visible (`pool: ... X at cap`), so it stops looking like a silent bug.
